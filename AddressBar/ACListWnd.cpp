...
void CACListWnd::OnPaint() {
  ...
  dc.BitBlt(m_rect.left, m_rect.top, m_rect.Width(), m_rect.Height(), &MemDC,
...
BOOL CACListWnd::OnEraseBkgnd(CDC * /*pDC*/) { return false; }
...
void CACListWnd::OnNcCalcSize(BOOL /*bCalcValidRects*/,
...
void CACListWnd::OnGetMinMaxInfo(MINMAXINFO FAR *lpMMI) {
  if (GetSafeHwnd()) {
    lpMMI->ptMinTrackSize.y =
        GetSystemMetrics(SM_CXHSCROLL) + GetSystemMetrics(SM_CYBORDER) * 2;
    lpMMI->ptMinTrackSize.x = GetSystemMetrics(SM_CXHSCROLL) * 4;
  } else
    CWnd::OnGetMinMaxInfo(lpMMI);
}

void CACListWnd::SortList(CStringAr &List) {
