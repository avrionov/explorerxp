...
//IMPLEMENT_FIXED_ALLOC(GRID_ROW,POOL_SIZE);
...
void CGridCtrl::SetupDefaultCells()
{
    ...
    //m_cellFixedRowDef.SetBackClr(m_crWindowColour);
...
void CGridCtrl::OnTimer(UINT_PTR nIDEvent)
{
  //  ASSERT(nIDEvent == WM_LBUTTONDOWN);
	...
	if (nIDEvent == IDTIMER_DEBOUNCE)
    {
        KillTimer(IDTIMER_DEBOUNCE);        
        TRACE(_T("Dragging\n"));
		//m_MouseMode = MOUSE_NOTHING;
		CPoint pt;
		GetCursorPos (&pt);
		ScreenToClient (&pt);
//		if ( (abs (pt.x - m_LeftClickDownPoint.x) > 2) || (abs (pt.y - m_LeftClickDownPoint.y) > 2))
//			OnBeginDrag ();
        return;
    }
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    //CPen pen;
    //pen.CreatePen(PS_DOT, 1, m_crGridLineColour);
    ...
    LogBrush.lbColor = m_crGridLineColour;
...
//	CWaitCursor wait; // Thomas Haase 
    ...
    if (bSelectCells)
    {
		...
		ClearSelection (nMinRow, nMinCol, nMaxRow, nMaxCol);
...
void CGridCtrl::ClearCells(CCellRange Selection)
{
    for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
    {
        for (int col = Selection.GetMinCol(); col <= Selection.GetMaxCol(); col++)
        {
            ...
            {
                ValidateAndModifyCellContents(row, col, EMPTYSTR);
            }
		}
	}
    Refresh();
...
void CGridCtrl::CutSelectedText()
{
    ...
    for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
    {
        ...
        ValidateAndModifyCellContents(cell.row, cell.col, EMPTYSTR);
    }
}
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
    ...
    do
    {
        ...
        CString strNext = (nIndex < 0)? EMPTYSTR  : strLine.Mid(nIndex + 1);
        ...
        while (!strLine.IsEmpty())
        {
            ...
            strLine = (nLineIndex >= 0)? strLine.Mid(nLineIndex + 1) : EMPTYSTR;
...
void CGridCtrl::ExpandLastColumn()
{
	...
	{
		if (abs (nDifference) < m_arColWidths[ nLastColumn ] )
		{
			m_arColWidths[ nLastColumn ] += nDifference;
			Refresh();
		}
...
void CGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
    ...
    if (m_bLMouseButtonDown)
    {
        ...
        case MOUSE_PREPARE_DRAG:
			...
			if ( (abs (point.x - m_LeftClickDownPoint.x) + abs (point.y - m_LeftClickDownPoint.y) ) > 2)
				OnBeginDrag ();            
            break;
#endif
        }    
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (pCell)
	{
		SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVN_LBUTTONDOWN);
    ...
    else if (IsCellSelected(m_LeftClickDownCell))
    {
		...
		{
			m_MouseMode = MOUSE_PREPARE_DRAG;
			SetTimer(IDTIMER_DEBOUNCE, IDTIMER_BUTTON_DELAY, NULL);
			//OnBeginDrag ();
		}
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    else if (m_MouseMode == MOUSE_PREPARE_DRAG) 
    {
        ...
        SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVN_LBUTTONUP);
	    SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVN_SELCHANGING);
        ResetSelectedRange();
	    SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVN_SELCHANGED);
		SelectRows (m_LeftClickDownCell, TRUE, TRUE);
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
    ...
    if(!IsValid(FocusCell))
        SendMessageToParent(-1, -1, GVN_RCLICK);
    else
    {
        SetFocusCell(-1,-1);
        ...
        SendMessageToParent(FocusCell.row, FocusCell.col, GVN_RCLICK);
    }
}
#endif
...
void CGridCtrl::ClearSelection (int nMinRow, int nMinCol, int nMaxRow, int nMaxCol, BOOL bForceRepaint)
{
	POSITION pos;
...
void CGridCtrl::ClearSelection ()
{
	ResetSelectedRange ();
}


void CGridCtrl::SaveCurrentCellCelection ()
{
	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
	{
		DWORD key;
...
void CGridCtrl::CreateSortIcons()
{
