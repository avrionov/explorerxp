...
IMPLEMENT_DYNAMIC(CShellTree, CTreeCtrl)

IContextMenu2* CShellTree::m_pContextMenu2 = NULL;
...
BEGIN_MESSAGE_MAP(CShellTree, CTreeCtrl)
  ...
  ON_WM_RBUTTONDOWN()
  ON_WM_DESTROY()
  ON_NOTIFY_REFLECT(TVN_ITEMEXPANDING, &CShellTree::OnItemexpanding)
  ON_NOTIFY_REFLECT(TVN_DELETEITEM, &CShellTree::OnDeleteitem)
  ON_MESSAGE(WM_MFC_INITCTRL, &CShellTree::OnInitControl)
END_MESSAGE_MAP()
...
void CShellTree::SetRelatedList(CMFCShellListCtrl* pShellList)
{
  /*
  ASSERT_VALID(this);
...
CMFCShellListCtrl* CShellTree::GetRelatedList() const
{
  ASSERT_VALID(this);
...
int CALLBACK CShellTree::CompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
...
void CShellTree::OnShowContextMenu(CPoint point)
{
...
CString CShellTree::OnGetItemText(LPAFX_SHELLITEMINFO pItem)
{
  ENSURE(pItem != NULL);
...
int CShellTree::OnGetItemIcon(LPAFX_SHELLITEMINFO pItem, BOOL bSelected)
{
  ENSURE(pItem != NULL);
...
BOOL CShellTree::SelectPath(LPCTSTR lpszPath, BOOL fExpandToShowChildren/* = TRUE */)
{
  ASSERT_VALID(this);
  ASSERT_VALID(afxShellManager);
...
BOOL CShellTree::SelectPath(LPCITEMIDLIST lpidl, BOOL fExpandToShowChildren/* = TRUE */)
{
  BOOL bRes = FALSE;
...
BOOL CShellTree::GetItemPath(CString& strPath, HTREEITEM htreeItem) const
{
  ASSERT_VALID(this);
...
void CShellTree::EnableShellContextMenu(BOOL bEnable)
{
  m_bContextMenu = bEnable;
}

BOOL CShellTree::OnChildNotify(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pLResult)
{
...
void CShellTree::PreSubclassWindow()
{
...
void CShellTree::SetFlags(DWORD dwFlags, BOOL bRefresh)
{
  ASSERT_VALID(this);
  m_dwFlags = dwFlags;
...
LRESULT CShellTree::OnInitControl(WPARAM wParam, LPARAM lParam)
{
