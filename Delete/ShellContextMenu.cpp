...
// NOTE: Slight change to return here but change it back if you want: PO
    ...
    hr = m_lpcm->QueryContextMenu(*pMenu, 0, IDM_SHELLCTXFIRST, IDM_SHELLCTXLAST, CMF_EXPLORE);
	/////////////////////////////
    ...
    for (int i = 0; i < count; i++)
    {
		...
		char verb_name[MAX_PATH]={0};

		HRESULT hr_command = m_lpcm->GetCommandString (mii.wID-IDM_SHELLCTXFIRST, GCS_VERBA, NULL, (LPTSTR)verb_name, 40);
		if (SUCCEEDED(hr_command))
			TRACE("%d %s\n", mii.wID, verb_name);
