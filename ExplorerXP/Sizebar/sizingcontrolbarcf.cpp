...
void CSizingControlBarCF::NcPaintGripper(CDC* pDC, CRect rcClient)
{
    ...
    if (!bGradient)
	{
		 //clrCptn = GetSysColor (COLOR_3DFACE);
		 //pDC->FillSolidRect(&rcGrip, clrCptn); // solid color
		 /*rcGrip.bottom++;
		 rcGrip.right = rcClient.right;
		 pDC->DrawEdge (&rcGrip, EDGE_ETCHED, BF_BOTTOM | BF_RIGHT );
		 rcGrip.bottom--;*/
	}
   ...
   // if (bFont)
    {
        ...
        pDC->DrawText(sTitle, &rcGrip, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        //"Hello!", -1, & rc, DT_CENTER | DT_VCENTER | DT_SINGLELINE
