...
**************************************************************/
{
	...
	{//completion port created/directory associated w/ it successfully
		...
		{
			//start the thread
			CWinThread * pThread = AfxBeginThread(MonitorDirectoryChanges, this, THREAD_PRIORITY_LOWEST);
			if( !pThread )
...
UINT __cdecl CDirectoryChangeWatcher::MonitorDirectoryChanges(LPVOID lpvThis)
/********************************************
    ...
    do
    {
		...
		{//The io completion request failed...
		 ...
		 //was used in a call to ReadDirectoryChangesW has been closed.
			...
			//  
			//
			if( !pdi || ( pdi && AfxIsValidAddress(pdi, sizeof(CDirectoryChangeWatcher::CDirWatchInfo)))
