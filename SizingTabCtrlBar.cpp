...
int CSizingTabCtrlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_tabctrl.Create(WS_VISIBLE | WS_CHILD | TCS_BOTTOM |WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_OVERLAPPED, 
		rect, this, IDI_TABCTRLBAR);
	
	m_tabctrl.MoveWindow(-1, rect.Height () -24, rect.Width () +20, 24);
...
void CSizingTabCtrlBar::OnSize(UINT nType, int cx, int cy) 
{
	...
	m_tabctrl.MoveWindow(-1, cy -24, cx+20, 24);
	CRect rc;
	GetWindowRect (&rc);
	TRACE(_T("%d %d\n"),rc.top, rc.left);
	CWnd *pWnd;
    for (POSITION pos=m_views.GetHeadPosition(); pos; m_views.GetNext(pos))
    {
         ...
         pWnd->MoveWindow(2, 2, cx - 5, bottom-10);
    }
	Invalidate ();
...
BOOL CSizingTabCtrlBar::IsHorz() const
{
...
int CSizingTabCtrlBar::GetActiveViewIndex ()
{
